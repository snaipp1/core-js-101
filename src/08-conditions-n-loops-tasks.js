...
function doRectanglesOverlap(rect1, rect2) {
  if (((rect1.top + rect1.height) > rect2.top && rect1.top <= rect2.top
  && (rect1.left + rect1.width) > rect2.left && rect1.left <= rect2.left)
  || ((rect2.top + rect2.height) > rect1.top && rect2.top <= rect1.top
  && (rect2.left + rect2.width) > rect1.left && rect2.left <= rect1.left)) return true;
  return false;
}
...
function isCreditCardNumber(ccn) {
  ...
  const result = arr.reduce((accum, item, idx) => {
    let itm = +item;
    if ((arr.length - idx) % 2 === 0) {
      itm *= 2;
      if (itm > 9) itm -= 9;
    }
    return accum + itm;
