...
function timeSpanToString(startDate, endDate) {
  ...
  return `${addZero(hours)}:${addZero(minutes)}:${addZero(seconds)}.${addZero(addZero(milliseconds))}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const hourNow = timeNow.getUTCHours() % 12;
  const minutesNow = timeNow.getUTCMinutes();
  const angleH = (360 / 12) * hourNow + (360 / (12 * 60)) * minutesNow;
  const angleM = (360 / 60) * minutesNow;
  let delta = Math.abs(angleH - angleM);
